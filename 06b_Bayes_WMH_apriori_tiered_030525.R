...
datadir <- "/Users/tinlizzy/Documents/professional/career/BUSPH/SPHERE/Bayes_MRI/"
...
inits <- list(alpha = 0, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, sigma = 1)
...
ocode <- nimbleCode({
  ...
  beta1 ~ dnorm(0.6, sd = 0.75) # prior for beta1 // depression
  beta2 ~ dnorm(0.01, sd = 0.1) # prior for beta2 // ICV offset [slightly larger volumes]
  beta3 ~ dnorm(0, sd = 1) # prior for beta3 // gender [no hypothesis for female gender?]
  beta4 ~ dnorm(0.1, sd = 0.5) # prior for beta4 // age [higher age, higher volume]
#  beta5 ~ dnorm(0.01, sd = 0.1) # prior for beta5 // time2mri [higher time, more volume, but small]
#  beta6 ~ dnorm(-0.1, sd = 0.5) # prior for beta6 //college [more college, less volume]
  sigma ~ dunif(1, 2) # prior for variance components
  
  # regression formula
  for (i in 1:no) {
    # n is the number of observations we have in the data
    mu[i] <- alpha+beta1*x1[i]+beta2*x2[i]+beta3*x3[i]+beta4*x4[i] # manual entry of linear predictors
...
odata <- list(y = dat$log_wmh_cen, x1 = dat$NIHTLBX_depr_theta, x2 = dat$c_offset_cen,
              x3 = dat$female, x4 = dat$age_cen)


#This will establish which samples will be used in the sampling of the posteriors
#It is not going to run the model yet
#If there is a conjugate relationship apparent between prior and posterior 
# (e.g., Normal-Normal, Binomial-Beta, Poisson-Gamma), `NIMBLE` will 
...
adata <- list(y = asian$log_wmh_cen, x1 = asian$NIHTLBX_depr_theta, x2 = asian$c_offset_cen,
              x3 = asian$female, x4 = asian$age_cen)
...
bdata <- list(y = black$log_wmh_cen, x1 = black$NIHTLBX_depr_theta, x2 = black$c_offset_cen,
              x3 = black$female, x4 = black$age_cen)
...
ldata <- list(y = latinx$log_wmh_cen, x1 = latinx$NIHTLBX_depr_theta, x2 = latinx$c_offset_cen,
              x3 = latinx$female, x4 = latinx$age_cen)
...
wdata <- list(y = white$log_wmh_cen, x1 = white$NIHTLBX_depr_theta, x2 = white$c_offset_cen,
              x3 = white$female, x4 = white$age_cen)
...
#######################
## ADJUSTED MODLES ##
# M2: M1 (age, ICV, sex) + college
#Constants
#Set initial values for MCMC samples (very important for convergence)
inits <- list(alpha = 0, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta6 = 0, sigma = 1)
...
#  beta5 ~ dnorm(0.01, sd = 0.1) # prior for beta5 // time2mri [higher time, more volume, but small]
  ...
  for (i in 1:no) {
    # n is the number of observations we have in the data
    mu[i] <- alpha+beta1*x1[i]+beta2*x2[i]+beta3*x3[i]+beta4*x4[i]+beta6*x6[i] # manual entry of linear predictors
    y[i] ~ dnorm(mu[i], sd = sigma)
  }
})
...
constants <- list(n = no)
odata <- list(y = dat$log_wmh_cen, x1 = dat$NIHTLBX_depr_theta, x2 = dat$c_offset_cen,
              x3 = dat$female, x4 = dat$age_cen, x6 = dat$college)
...
constants <- list(n = na)
adata <- list(y = asian$log_wmh_cen, x1 = asian$NIHTLBX_depr_theta, x2 = asian$c_offset_cen,
              x3 = asian$female, x4 = asian$age_cen, x6 = asian$college)
...
constants <- list(n = nb)
bdata <- list(y = black$log_wmh_cen, x1 = black$NIHTLBX_depr_theta, x2 = black$c_offset_cen,
              x3 = black$female, x4 = black$age_cen, x6 = black$college)
...
constants <- list(n = n)
ldata <- list(y = latinx$log_wmh_cen, x1 = latinx$NIHTLBX_depr_theta, x2 = latinx$c_offset_cen,
              x3 = latinx$female, x4 = latinx$age_cen, x6 = latinx$college)
...
constants <- list(n = n)
wdata <- list(y = white$log_wmh_cen, x1 = white$NIHTLBX_depr_theta, x2 = white$c_offset_cen,
              x3 = white$female, x4 = white$age_cen, x6 = white$college)
...
#######################
## ADJUSTED MODLES ##
# M3: M2 (age, ICV, sex, college) + time2mri
