...
ocode <- nimbleCode({
  ...
  beta1 ~ dnorm(0.6, sd = 1.5) # prior for beta1 // depression
...
ocode <- nimbleCode({
  # priors for parameters
  alpha ~ dnorm(0, sd = 1) # prior for alpha
  beta1 ~ dnorm(0.1, sd = 0.75) # prior for beta1 // depression
  beta2 ~ dnorm(0.01, sd = 0.1) # prior for beta2 // ICV offset [slightly larger volumes]
  beta3 ~ dnorm(0, sd = 1) # prior for beta3 // gender [no hypothesis for female gender?]
  beta4 ~ dnorm(0.1, sd = 0.5) # prior for beta4 // age [higher age, higher volume]
  beta5 ~ dnorm(0.01, sd = 0.1) # prior for beta5 // time2mri [higher time, more volume, but small]
...
ocode <- nimbleCode({
  # priors for parameters
  alpha ~ dnorm(0, sd = 1) # prior for alpha
  beta1 ~ dnorm(1.1, sd = 0.75) # prior for beta1 // depression
  beta2 ~ dnorm(0.01, sd = 0.1) # prior for beta2 // ICV offset [slightly larger volumes]
  beta3 ~ dnorm(0, sd = 1) # prior for beta3 // gender [no hypothesis for female gender?]
  beta4 ~ dnorm(0.1, sd = 0.5) # prior for beta4 // age [higher age, higher volume]
  beta5 ~ dnorm(0.01, sd = 0.1) # prior for beta5 // time2mri [higher time, more volume, but small]
...
write_csv(sumsx, paste0(datadir, "Results/Log_WMH_Sensitivity_Posteriors.csv"))
